...
def preprocess_single(
    fn, satname, cloud_mask_issue=False, pan_off=False, collection='C02', do_cloud_mask=False, s2cloudless_prob=60
):
    """
    ...
    if satname == "L5":
        ...
        idx = basename.find('_ms')
        name = basename[0:idx]
        fn_ms = fn
        fn_mask = os.path.join(os.path.dirname(os.path.dirname(fn)), 'mask', name+'_mask.tif')
        # read ms bands
    ...
    # =============================================================================================#
        ...
        name = basename[0:idx]
        fn_ms = fn
        fn_pan = os.path.join(os.path.dirname(os.path.dirname(fn)), 'pan', name+'_pan.tif')
        fn_mask = os.path.join(os.path.dirname(os.path.dirname(fn)), 'mask', name+'_mask.tif')
        # read ms bands
        data = gdal.Open(fn_ms, gdal.GA_ReadOnly)
        georef = np.array(data.GetGeoTransform())
    ...
    if satname == "S2":
        ...
        fn_swir = os.path.join(os.path.dirname(os.path.dirname(fn)), 'swir', name+'_swir.tif')
