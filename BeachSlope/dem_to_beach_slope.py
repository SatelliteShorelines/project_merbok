...
# parser.add_argument("-g", "--global_region", type=str)
# parser.add_argument("-c", "--coastal_area", type=str)
# parser.add_argument("-rr", "--subregion", type=str)
# parser.add_argument("-sss", "--shoreline_section", type=str,required=False, default = '')
# parser.add_argument("-d", "--dem", type=str,required=False, default = '')

# args = parser.parse_args()

def epsg_from_utm(zone, northern=True):
    """
    Returns the EPSG code for a given UTM zone.

    Args:
        zone (int): UTM zone number.
        northern (bool, optional): True if the zone is in the northern hemisphere, 
                                   False for southern hemisphere. Defaults to True.
...
def make_profile_plot(csv_file, NO_DATA, res=2, clip_land=0, vertical_datum='WGS84 Ellipsoid'):
    ...
    try:
        ...
        ##approximate foreshore as transect portion between inflection point and toe
        points = [inflection_point, toe]
    ...
    try:
        ...
        plt.legend(loc='best')
        plt.tight_layout()
        plt.savefig(os.path.splitext(csv_file)[0]+'_profile_plot.png', dpi=300)
        plt.close('all')
    except:
        pass
...
def main(in_raster, in_line, csv_file, NO_DATA, batch=False, vertical_datum = 'WGS84 Ellipsoid'):
    """
    ...
    if ds is None:
        ...
        sys.exit(1)
    _, res, _, _, _, _ = ds.GetGeoTransform()
    # get raster bands
    ...
    distance = []
    for currentdistance in range(0, int(length_m), int(res)):
...
def batch_main(in_raster, in_lines_path, out_folder, section_string, v, crs=6393, vertical_datum='WGS84 Ellipsoid'):
    """
    ...
    for feature in layer:
        ...
        try:
            output_dict = main(in_raster, feature, csv_path, NO_DATA, batch=True, vertical_datum=vertical_datum)
            ...
            max_slopes[i] = np.nan
            max_tan_betas[i] = np.nan
            avg_slopes[i] = np.nan
            avg_tan_betas[i] = np.nan
            median_slopes[i] = np.nan
            median_tan_betas[i] = np.nan
            ip_xs[i] = np.nan
            ip_ys[i] = np.nan
            crest_xs[i] = np.nan
            crest_ys[i] = np.nan
    ...
    def convert_cols(df, xcol, ycol):
        lon, lat = transformer.transform(df[xcol].values, df[ycol].values)
        df[xcol.replace("_x","_lon")] = lon
        df[ycol.replace("_y","_lat")] = lat

    pairs = [
        ("inflection_x", "inflection_y"),
        ("crest_x", "crest_y"),
        ("crest2_x", "crest2_y"),
        ("crest3_x", "crest3_y"),
        ("toe_x", "toe_y"),
    ...
    for xcol, ycol in pairs:
        convert_cols(df, xcol, ycol)

    df.to_csv(os.path.join(os.path.dirname(out_folder), section_string+'_slopes.csv'))

    ##remove temporary shapefile
    shutil.rmtree(shp_folder,  ignore_errors=True)
    
    return df

def batch_main_custom(site, in_raster, in_lines_path, out_folder, crs=6393, vertical_datum='WGS84 Ellipsoid'):
    """
    Repeatedly take elevation profiles from a raster dem with an input shapefile containing all of the lines
    inputs:
    site (str): site name
...
def profile_shoreline_section(G,
    ...
    custom_dem (str): type of custom dem
    ...
    for section in sections:
        ...
        if arctic_dem == True:
            res=2
            crs=3413
            raster_path = os.path.join(section_dir, 'DEMs', 'Arctic_DEM', section_string+'_arctic_dem.tif')
            dem_name = ''
        elif custom_dem == 'AlaskaDSM':
            try:
                raster_path = glob.glob(os.path.join(section_dir, 'DEMs', 'AlaskaDSM')+'/*.tif')[0]
                dem_name = 'AlaskaDSM'
            except:
                print('no AlaskaDSM')
                continue
        elif custom_dem == 'TBDEM':
            try:
                raster_path = glob.glob(os.path.join(section_dir, 'DEMs', 'TBDEM')+'/*.tif')[0]
                dem_name = 'TBDEM'
            except:
                print('no TBDEM')
                continue
        elif custom_dem == 'JABLTCX':
            try:
                raster_path = glob.glob(os.path.join(section_dir, 'DEMs', 'JABLTCX')+'/*.tif')[0]
                dem_name = 'JABLTCX'
            except:
                print('no JABLTCX')
                continue
        elif custom_dem == 'FODAR':
            try:
                raster_paths = glob.glob(os.path.join(section_dir, 'DEMs', 'FODAR')+'/*.tif')
                if len(raster_paths)==0:
                    print('no FODAR')
                    continue
                vrt = gdal.BuildVRT(os.path.join(section_dir, 'DEMs', 'FODAR', 'vrt.vrt'), raster_paths)
                raster_path = os.path.join(section_dir, 'DEMs', 'FODAR', 'vrt.vrt')
                dem_name = 'FODAR'
            except:
                print('no FODAR')
                continue
        ...
        try:
            ...
            try:
                if crs.to_epsg()!=None:
                    ...
                    crs = crs.to_dict()
                    crs = int(crs['zone'])
                    crs = epsg_from_utm(crs, northern=True)
            except:
                crs = crs.to_dict()
                crs = int(crs['zone'])
                crs = epsg_from_utm(crs, northern=True)

        out_folder = os.path.join(section_dir, 'elevation_profiles_'+dem_name)
        out_lines_folder = os.path.join(section_dir, 'elevation_profile_lines_'+dem_name)
        try:
            os.mkdir(out_folder)
        except:
            pass
        try:
            os.mkdir(out_lines_folder)
        except:
            pass
        
        df = batch_main(raster_path, transects_path, out_folder, section_string, V, crs=crs, vertical_datum=vertical_datum)
        ...
        try:
            ...
            ip_lines_path = os.path.join(out_lines_folder, section_string + '_inflection_points.geojson')
            ...
            crest_lines_path = os.path.join(out_lines_folder, section_string + '_crest_points.geojson')
            ...
            crest2_lines_path = os.path.join(out_lines_folder, section_string + '_crest2_points.geojson')
            ...
            crest3_lines_path = os.path.join(out_lines_folder, section_string + '_crest3_points.geojson')
            ...
            toe_lines_path = os.path.join(out_lines_folder, section_string + '_toe_points.geojson')
            toe_lines.to_file(toe_lines_path)
            toe_lines = smooth_lines(toe_lines_path)


            df.to_csv(os.path.join(os.path.dirname(out_folder), section_string+'_slopes_'+dem_name+'.csv'))
            ...
            transects_df.to_file(os.path.splitext(transects_path)[0]+'_slopes_'+dem_name+'.geojson')
        except:
            pass
...
def generic_profile(site,
    ...
    ##3413 is polar stereo north
    try:
        os.mkdir(os.path.join(output_folder,'elevation_profile_lines'))
    except:
        pass
    ip_points = gpd.GeoDataFrame(df_no_nans,
                                 geometry=gpd.points_from_xy(df_no_nans['inflection_x'],
                                 ...
                                 crs=crs)
    ip_lines = geojson_points_to_lines(ip_points, crs)
    ip_lines_path = os.path.join(output_folder,'elevation_profile_lines',  site + '_inflection_points.geojson')
    ip_lines.to_file(ip_lines_path)
    ip_lines = smooth_lines(ip_lines_path)
    ...
    crest_points = gpd.GeoDataFrame(df_no_nans,
                                  ...
                                  crs=crs)
    crest_lines = geojson_points_to_lines(crest_points, crs)
    crest_lines_path = os.path.join(output_folder, 'elevation_profile_lines', site +'_crest_points.geojson')
    crest_lines.to_file(crest_lines_path)
    crest_lines = smooth_lines(crest_lines_path)
    ...
    crest2_points = gpd.GeoDataFrame(df_no_nans,
                                  ...
                                  crs=crs)
    crest2_lines = geojson_points_to_lines(crest2_points, crs)
    crest2_lines_path = os.path.join(output_folder, 'elevation_profile_lines', site + '_crest2_points.geojson')
    crest2_lines.to_file(crest2_lines_path)
    crest2_lines = smooth_lines(crest2_lines_path)
    ...
    crest3_points = gpd.GeoDataFrame(df_no_nans,
                                  ...
                                  crs=crs)
    crest3_lines = geojson_points_to_lines(crest3_points, crs)
    crest3_lines_path = os.path.join(output_folder, 'elevation_profile_lines', site + '_crest3_points.geojson')
    crest3_lines.to_file(crest3_lines_path)
    crest3_lines = smooth_lines(crest3_lines_path)
    ...
    toe_points = gpd.GeoDataFrame(df_no_nans,
                                  ...
                                  crs=crs)
    toe_lines = geojson_points_to_lines(toe_points, crs)
    toe_lines_path = os.path.join(output_folder, 'elevation_profile_lines', site + '_toe_points.geojson')
    toe_lines.to_file(toe_lines_path)
    toe_lines = smooth_lines(toe_lines_path)
...
# if args.shoreline_section != '':
#     sss = args.shoreline_section
#     profile_shoreline_section(g,
#                               c,
#                               rr,
#                               v,
#                               r_home,
